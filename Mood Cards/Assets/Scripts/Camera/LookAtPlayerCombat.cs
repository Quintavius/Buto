...
public class LookAtPlayerCombat : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //Average position between player and enemy
        var posx = (player.transform.position.x + enemy.transform.position.x) / 2;
        var posy = (player.transform.position.y + enemy.transform.position.y) / 2;
        var posz = (player.transform.position.z + enemy.transform.position.z) / 2;
        Vector3 averagePos = new Vector3(posx, posy, posz);
        ...
        if (follow)
        {
            var tx = Mathf.SmoothDamp(tracker.transform.position.x, posx + offset.x, ref trackVelocity, hardness);
            var ty = Mathf.SmoothDamp(tracker.transform.position.y, posy + offset.y, ref trackVelocity, hardness);
            var tz = Mathf.SmoothDamp(tracker.transform.position.z, posz + offset.z, ref trackVelocity, hardness);
            tracker.transform.position = new Vector3(tx, ty, tz);
        } else
        {
            offset = tracker.transform.position - averagePos;
        }
        ...
        if (allowLiveTracking)
        {
            ...
            tracker.transform.position = Vector3.MoveTowards(tracker.transform.position, averagePos, step * offscreenModifier);
        }
