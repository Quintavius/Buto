...
public class FaceAnimatorStandard : MonoBehaviour {
    ...
    private void LateUpdate()
    {
        ...
        faceMat.SetTextureOffset("_MainTex", new Vector2(offset.x, offset.y));
    ...
    void Look()
    {
        ...
        if (angle < 75)
        {
            //hacky way of getting smooth look at, use LookAt only for standard
            ...
            lastLook = headBone.transform.rotation;                                                                                             //store current for next round
        }
        else
        {
            ...
            //headBone.transform.rotation = Quaternion.Lerp(lastLook, nextLook, Time.deltaTime * 2); //this one will lerp to last known place
